...
mod tests {
    ...
    fn test_component_definition_with_dependencies() {
        ...
        let creator_fn = |deps: HashMap<TypeId, ComponentInstance>| {
            let simple_dep = deps
                ...
                .ok_or_else(|| {
                    verdure_core::error::component::ComponentError::DependencyNotFound(
                        "SimpleComponent".to_string(),
                    )
                })?
                .clone()
                .downcast::<SimpleComponent>()
                .map_err(|_| {
                    verdure_core::error::component::ComponentError::DowncastFailed(
                        "SimpleComponent".to_string(),
                    )
                })?;
    ...
    fn test_component_definition_creator_missing_dependency() {
        let creator_fn = |deps: HashMap<TypeId, ComponentInstance>| {
            let _simple_dep = deps.get(&TypeId::of::<SimpleComponent>()).ok_or_else(|| {
                verdure_core::error::component::ComponentError::DependencyNotFound(
                    "SimpleComponent".to_string(),
                )
            })?;
        ...
        if let Err(error) = result {
            assert!(matches!(
                error,
                verdure_core::error::component::ComponentError::DependencyNotFound(_)
            ));
        }
    }
}
