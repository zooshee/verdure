...
pub trait AnyContextAwareEventListener: Send + Sync {
    ...
    /// 
    /// `true` if the listener handled the event, `false` otherwise
    fn handle_context_event(&self, event: &dyn Event, context: &crate::context::ApplicationContext) -> bool;
...
impl<T: Event + 'static, L: ContextAwareEventListener<T>> AnyContextAwareEventListener for TypedContextAwareEventListener<T, L> {
    fn handle_context_event(&self, event: &dyn Event, context: &crate::context::ApplicationContext) -> bool {
...
impl EventPublisher {
    ...
    /// ```
    pub fn subscribe_context_aware<T: Event + 'static, L: ContextAwareEventListener<T> + 'static>(&mut self, listener: L) {
    ...
    /// * `context` - Reference to the ApplicationContext
    pub fn publish_with_context<T: Event + 'static>(&self, event: &T, context: &crate::context::ApplicationContext) {
        let type_id = TypeId::of::<T>();
...
mod tests {
    ...
    fn test_different_event_types() {
        ...
        let another_event = AnotherEvent {
            value: 42,
        };
