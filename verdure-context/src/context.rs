...
use crate::event::{EventPublisher, Event, EventListener, ContextAwareEventListener, ContextInitializingEvent, ContextInitializedEvent, ProfileActivatedEvent, ConfigurationChangedEvent};
...
impl ApplicationContextBuilder {
    ...
    pub fn with_properties_config_file<P: AsRef<Path>>(mut self, path: P) -> Self {
        let path_str = path.as_ref().to_string_lossy().to_string();
        self.config_sources.push(ConfigSource::PropertiesFile(path_str));
        self
    }
    ...
    pub fn build(self) -> ContextResult<ApplicationContext> {
        ...
        if !self.properties.is_empty() {
            context.config_manager.add_source(ConfigSource::Properties(self.properties))?;
        }
        
        // Add profiles
        for profile in self.profiles {
            context.config_manager.profile_manager_mut().add_profile(profile)?;
        }
        
        // Activate profiles
        for profile_name in self.active_profiles {
            context.config_manager.profile_manager_mut().activate_profile(&profile_name)?;
            
            // Get properties count for the activated profile
            let properties_count = context.config_manager.profile_manager().get_profile_properties_count(&profile_name);
...
impl ApplicationContext {
    ...
    pub fn initialize(&self) -> ContextResult<()> {
        ...
        let initializing_event = ContextInitializingEvent {
            ...
            active_profiles_count: self.config_manager.profile_manager().active_profiles().len(),
            timestamp: std::time::SystemTime::now(),
        };
        self.event_publisher.publish_with_context(&initializing_event, self);
        
        // Initialize the IoC container
        self.container.initialize()
            .map_err(|e| ContextError::initialization_failed(format!("IoC container initialization failed: {}", e)))?;
        ...
        self.event_publisher.publish_with_context(&initialized_event, self);
    ...
    pub fn set_config(&mut self, key: &str, value: &str) {
        ...
        let old_value_opt = if old_value.is_empty() { None } else { Some(old_value) };
        
        // Set the new configuration value
        self.config_manager.set(key, ConfigValue::String(value.to_string()));
    ...
    /// ```
    pub fn subscribe_to_context_events<T: Event + 'static, L: ContextAwareEventListener<T> + 'static>(&mut self, listener: L) {
    ...
    /// ```
    pub fn subscribe_to_events<T: Event + 'static, L: EventListener<T> + 'static>(&mut self, listener: L) {
...
mod tests {
    ...
    fn test_configuration_management() {
        ...
        let mut props = HashMap::new();
        props.insert("database.url".to_string(), "postgres://localhost/test".to_string());
        props.insert("server.port".to_string(), "3000".to_string());
        props.insert("debug.enabled".to_string(), "true".to_string());
        
        context.add_config_source(ConfigSource::Properties(props)).unwrap();
        
        assert_eq!(context.get_config("database.url"), "postgres://localhost/test");
    ...
    fn test_configuration_with_defaults() {
        ...
        assert_eq!(context.get_config_or_default("missing.key", "default"), "default");
    }
    ...
    fn test_built_in_context_events() {
        ...
        use crate::event::{ContextInitializingEvent, ContextInitializedEvent, ProfileActivatedEvent, ConfigurationChangedEvent};
        ...
        assert_eq!(config_events[1].old_value, Some("initial.value".to_string()));
    ...
    fn test_context_aware_event_listeners() {
        use std::sync::{Arc, Mutex};
        use crate::event::{ContextInitializedEvent, ContextAwareEventListener};
        ...
        impl ContextAwareEventListener<ContextInitializedEvent> for ContextAwareListener {
            fn on_context_event(&self, event: &ContextInitializedEvent, context: &crate::context::ApplicationContext) {
                // Record the event
                let mut events = self.events.lock().unwrap();
                events.push(format!("Initialized with {} sources", event.config_sources_count));
